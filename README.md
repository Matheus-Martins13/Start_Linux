...
Distribuições Linux, também chamadas de distros, são os sistemas operacionais distribuídos por alguma empresa ou pessoa, para alcançar determinado fim. Existem distribuições Linux para segurança da informação, para uso em desktop, servidores, dentre muitos outros fins. Cada empresa pode decidir por colocar características próprias em cada distribuição, ou utilizar como base uma distro já pronta (algo que acontece muito no mundo linux). O que todas as distribuições têm em comum, é que todas utilizam o Linux como kernel. Como eu disse, existem centenas de distribuições Linux, seja para uso em Desktop, servidores ou outros fins. Exemplos: ArchLinux, Linux Mint, Debian, Fedora, Ubuntu, Zorin OS, Pop! OS, Gentoo, dentre muitas outras. As distribuições Linux são de código aberto e, em sua grande maioria, gratuitas. 
